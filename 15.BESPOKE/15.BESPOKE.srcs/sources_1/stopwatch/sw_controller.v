...
module sw_controller(
    ...
    // mode check, btn[0]
    always @(posedge clk or posedge reset or negedge sw_enable) begin
        if (reset || !sw_enable) begin
            r_mode <= 0;
            prev_btn[0] <= 0;
        end else begin
            if (btn[0] && !prev_btn[0]) begin // 처음 눌려진 상태
    ...
    always @(posedge clk or posedge reset or negedge sw_enable) begin
        if (reset || !sw_enable) begin
            ...
            if (btn[1] && !prev_btn[1]) begin  // 처음 눌려진 상태
    ...
    always @(posedge clk or posedge reset or negedge sw_enable) begin
        if (reset || !sw_enable) begin
            ...
            if (btn[2] && !prev_btn[2]) begin // 처음 눌려진 상태
                clear <= 1'b1;
            end else begin
                clear <= 1'b0;           
            end
    ...
    always @(posedge clk or posedge reset or negedge sw_enable) begin
        if (reset || !sw_enable) begin
            ...
            if (btn[3] && !prev_btn[3]) begin // 처음 눌려진 상태
                if (speed_idx == 2'd3)
                    speed_idx <= 0;
                else begin
